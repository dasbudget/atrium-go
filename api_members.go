...
/*
MembersApiService Read OAuth Window URI
This endpoint will generate an &#x60;oauth_window_uri&#x60; for the specified &#x60;member&#x60;.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param memberGUID The unique identifier for a &#x60;member&#x60;.
 * @param userGUID The unique identifier for a &#x60;user&#x60;.
 * @param optional nil or *ReadOAuthWindowURIOpts - Optional Parameters:
     * @param "ReferralSource" (optional.String) -  Should be either BROWSER or APP depending on the implementation.
     * @param "UiMessageWebviewURLScheme" (optional.String) -  A scheme for routing the user back to the application state they were previously in.

@return MemberResponseBody
*/

type ReadOAuthWindowURIOpts struct {
	ReferralSource            optional.String
	UiMessageWebviewURLScheme optional.String
}

func (a *MembersApiService) ReadOAuthWindowURI(ctx context.Context, memberGUID string, userGUID string, localVarOptionals *ReadOAuthWindowURIOpts) (MemberResponseBody, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Get")
		localVarPostBody    interface{}
		localVarFileName    string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/users/{user_guid}/members/{member_guid}/oauth_window_uri"
	localVarPath = strings.Replace(localVarPath, "{"+"member_guid"+"}", fmt.Sprintf("%v", memberGUID), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"user_guid"+"}", fmt.Sprintf("%v", userGUID), -1)
	...
	if localVarOptionals != nil && localVarOptionals.ReferralSource.IsSet() {
		localVarQueryParams.Add("referral_source", parameterToString(localVarOptionals.ReferralSource.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.UiMessageWebviewURLScheme.IsSet() {
		localVarQueryParams.Add("ui_message_webview_url_scheme", parameterToString(localVarOptionals.UiMessageWebviewURLScheme.Value(), ""))
	}
	// to determine the Content-Type header
	localVarHttpContentTypes := []string{}
